...
def justthefix(tryfix):
    ...
    questdig = sum(c.isdigit() for c in tryfix)
    #  Used later in case the call sign only has one digit.
    ...
    prefix = reversethis(prefix)
    del prefix[-1]
    # set the suffix to read forward.
    suffix = reversethis(suffix)
...
def printinput1():
    ...
    strprefix, strsepdig, strsuffix = justthefix(listinp)
    # Now let's find out where that call is from
    ourcountry = ctyfileread(strprefix, strsepdig)
